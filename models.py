...
class Bolt(GRectangle):
    ...
    #super for initiailizer, for parameters, self, x, y, velocity)
    #method to determine if player bolts
    #if positive bolt, return true else false
    #detect if bolt currently on offscreen
    #shoot bolt method
    #move bolt if the bolt is gretaer than the game height, remove the bolt
    #for loops to acces the y attribute, otherwise, increase y coordinate by bolt velocity
    #if the player is space space and there is no bbolt on screen, shoot a bolt
    #volt is an empty list,
    #firebotl takes in self and input
    #movebolt checks if bolt is greater than screen or nt, append
    #x and y coordinates are those of the ship
